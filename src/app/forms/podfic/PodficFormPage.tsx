...
import PodficForm from './podfic-form';
import { PodficType, SectionType } from '@/app/types';
import { useRouter } from 'next/navigation';

export default function PodficFormPage({ podficId = null, returnUrl = null }) {
  ...
  useEffect(() => {
    fetch(`/db/podfics/${podficId}?with_podficcers=true&with_tags=true`).then(
      (res) => {
        res.json().then((data) => {
          if (Object.keys(data).length && !!podficId) {
            console.log({ data });
            setPodfic(data);
          }
          setLoading(false);
